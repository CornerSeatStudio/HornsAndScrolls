...
public class FoliagePhysics : MonoBehaviour
{
    ...
    void Start() {
        ...
        radius = Mathf.Max(rend.bounds.max.x, rend.bounds.max.z) - (rend.bounds.center.x + rend.bounds.center.z)/2;
        Debug.Log(radius);
    ...
    IEnumerator GrassHandle(){
        while (true){
            ...
            if(Physics.OverlapSphere(transform.position, radius/2, foliageMask).Length > 0) {
